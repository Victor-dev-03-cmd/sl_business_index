  ...
  return (
    <div className="flex flex-col gap-6 p-4">
      ...
      <div className="flex items-center gap-3 p-3 bg-slate-900/60 border border-slate-800 rounded-2xl">
         ...
         <button 
           onClick={() => { if (canvas) { canvas.remove(activeObject); canvas.discardActiveObject(); canvas.renderAll(); updateLayers(); saveHistory(); } }}
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => { if (canvas) { canvas.bringObjectForward(activeObject); canvas.renderAll(); updateLayers(); saveHistory(); } }}
          ...
          <button
            onClick={() => { if (canvas) { canvas.sendObjectBackwards(activeObject); canvas.renderAll(); updateLayers(); saveHistory(); } }}
            className="flex items-center justify-center gap-2 py-3 bg-slate-900/60 hover:bg-slate-800 border border-slate-800 rounded-xl text-[10px] font-bold text-slate-300 transition-all"
          >
