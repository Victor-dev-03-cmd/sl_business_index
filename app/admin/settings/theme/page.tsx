  ...
  const { data: settings, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('site_settings')
        .select('id, theme_dark_color, theme_blue_color, theme_gold_color, theme_gold_light_color, theme_sand_color, theme_text_color')
        .eq('id', 1)
        .single();
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-normal text-gray-900 flex items-center gap-3">
            <Palette className="h-6 w-6 text-brand-blue" /> Theme Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">Customize global brand colors for buttons, icons, and text.</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={resetToDefault}
            className="flex items-center gap-2 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-[6px] transition-all text-sm font-bold"
          >
          ...
          <div className="bg-gray-50/30 p-6 rounded-xl border border-gray-100 space-y-6">
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Primary Dark Color
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_dark_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_dark_color || '#053765'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_dark_color: e.target.value} : null)}
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Brand Blue Color
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_blue_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_blue_color || '#2a7db4'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_blue_color: e.target.value} : null)}
                  className="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer overflow-hidden p-0"
                />
                <input 
                  type="text" 
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Main Text Color
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_text_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_text_color || '#124272'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_text_color: e.target.value} : null)}
                  className="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer overflow-hidden p-0"
                />
                <input 
                  type="text" 
        ...
        <section className="space-y-6">
          ...
          <div className="bg-gray-50/30 p-6 rounded-xl border border-gray-100 space-y-6">
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Primary Gold Color
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_gold_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_gold_color || '#b4863b'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_gold_color: e.target.value} : null)}
                  className="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer overflow-hidden p-0"
                />
                <input 
                  type="text" 
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Gold Light Color
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_gold_light_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_gold_light_color || '#c09a54'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_gold_light_color: e.target.value} : null)}
                  className="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer overflow-hidden p-0"
                />
                <input 
                  type="text" 
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-gray-700 flex justify-between">
                Sand Color (Accents)
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-widest">{localSettings?.theme_sand_color}</span>
              </label>
              <div className="flex items-center gap-4">
                <input 
                  type="color" 
                  value={localSettings?.theme_sand_color || '#dfb85d'}
                  onChange={(e) => setLocalSettings(s => s ? {...s, theme_sand_color: e.target.value} : null)}
                  className="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer overflow-hidden p-0"
                />
                <input 
                  type="text" 
      ...
      <section className="space-y-6 mt-12">
        ...
        <div className="bg-white p-12 rounded-xl border border-gray-300 shadow-inner flex flex-wrap items-center justify-center gap-8">
          ...
          <div className="flex items-center gap-3">
             <Palette size={24} style={{ color: localSettings?.theme_blue_color }} />
             <span style={{ color: localSettings?.theme_text_color }} className="font-bold text-lg">Sample Branding Text</span>
          </div>
        </div>
      </section>
    </div>
