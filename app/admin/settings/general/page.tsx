  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-normal text-gray-900 flex items-center gap-3">
            <Globe className="h-6 w-6 text-brand-blue" /> General Information
          </h1>
          <p className="text-sm text-gray-500 mt-1">Manage core site identity, brand assets, and platform status.</p>
        </div>
        <button 
          onClick={handleSave}
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="bg-gray-50/50 p-6 rounded-xl border border-dashed border-gray-200">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Search Preview</h3>
            <div className="space-y-1.5">
              ...
              <p className="text-[#545454] text-sm line-clamp-2 leading-snug">
                {localSettings?.site_description || 'Add a site description to see how it appears in search results.'}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm space-y-10">
          <div className="max-w-md space-y-2">
            <label className="text-sm font-bold text-gray-700">Logo Text</label>
            <p className="text-xs text-gray-400 mb-3">Fallback text when logo image is not available.</p>
            <input 
              type="text" 
          ...
          <div className="space-y-6">
            <label className="text-sm font-bold text-gray-700">Logo Configuration</label>
            <div className="flex flex-col xl:flex-row items-start gap-12">
              <div className="relative group shrink-0">
                <div className="h-48 w-80 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-brand-blue shadow-inner group-hover:bg-white">
                  {localSettings?.logo_url ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-gray-300">
                      ...
                      <span className="text-[10px] mt-3 font-bold uppercase tracking-[0.2em]">Upload Brand Logo</span>
                    </div>
                  )}
                  {isUploading && (
              ...
              <div className="flex-1 space-y-8 w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <div className="flex justify-between text-xs font-bold text-gray-600">
                      <span className="uppercase tracking-[0.1em]">Width Control</span>
                      <span className="text-brand-blue bg-brand-blue/5 px-2 py-0.5 rounded">{localSettings?.logo_width || 150}px</span>
                    </div>
                    <input 
                      type="range" 
                      min="50" 
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between text-xs font-bold text-gray-600">
                      <span className="uppercase tracking-[0.1em]">Height Control</span>
                      <span className="text-brand-blue bg-brand-blue/5 px-2 py-0.5 rounded">{localSettings?.logo_height || 50}px</span>
                    </div>
                    <input 
                      type="range" 
                      min="20" 
                ...
                <div className="p-4 bg-blue-50/50 rounded-lg border border-blue-100 flex gap-3">
                  ...
                  <p className="text-xs text-blue-800 leading-relaxed">
                    Adjust the dimensions to fit your site header perfectly. Recommended ratio is 3:1.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm space-y-8">
          <div className="flex items-center justify-between p-6 bg-gray-50 rounded-xl border border-gray-200 transition-all hover:bg-white hover:shadow-md group">
            <div className="flex items-center gap-5">
              <div className={`p-4 rounded-xl transition-all shadow-sm ${localSettings?.maintenance_mode ? 'bg-amber-500 text-white shadow-amber-200' : 'bg-emerald-500 text-white shadow-emerald-200'}`}>
                {localSettings?.maintenance_mode ? <AlertCircle size={24} /> : <Globe size={24} />}
              </div>
              <div>
                <p className="text-base font-bold text-gray-900">Maintenance Mode</p>
                <p className="text-sm text-gray-500 mt-0.5">Control public access to the entire platform.</p>
              </div>
            </div>
            <button 
              onClick={() => setLocalSettings(s => s ? {...s, maintenance_mode: !s.maintenance_mode} : null)}
              className={`w-16 h-8 rounded-full transition-all relative shadow-inner ${localSettings?.maintenance_mode ? 'bg-brand-blue' : 'bg-gray-300'}`}
            >
              <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all shadow-md ${localSettings?.maintenance_mode ? 'right-1' : 'left-1'}`} />
            </button>
          </div>
          ...
          {localSettings?.maintenance_mode && (
            <div className="space-y-3 animate-in slide-in-from-top-4 duration-300">
              <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                <AlertCircle size={14} className="text-amber-500" /> Maintenance Message
              </label>
              <textarea 
                rows={4}
                value={localSettings?.maintenance_message || ''}
                onChange={(e) => setLocalSettings(s => s ? {...s, maintenance_message: e.target.value} : null)}
              ...
              />
              <p className="text-[11px] text-amber-600 font-medium">This message will be displayed to all users except Administrators.</p>
            </div>
          )}
        </div>
      </section>
