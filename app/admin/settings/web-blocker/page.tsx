      ...
      <div className="flex items-center justify-between pb-6 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-normal text-gray-900 flex items-center gap-3">
            <ShieldAlert className="h-6 w-6 text-red-600" /> Web Blocker & Security
          </h1>
          <p className="text-sm text-gray-500 mt-1">Configure spam protection and IP-level access controls.</p>
        </div>
        <button 
          onClick={handleSave}
          ...
          <div className="bg-gray-50/30 rounded-xl border border-gray-100 divide-y divide-gray-100">
            <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-normal">Enable AI-driven analysis for reviews and messages to block spam patterns.</p>
                </div>
              </div>
              <button 
                onClick={() => setLocalSettings(s => s ? {...s, spam_protection_enabled: !s.spam_protection_enabled} : null)}
                className={`w-14 h-7 rounded-full transition-colors relative ${localSettings?.spam_protection_enabled ? 'bg-red-600' : 'bg-gray-200'}`}
              >
                <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${localSettings?.spam_protection_enabled ? 'right-1' : 'left-1'}`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-900">IP-Level Blocking</p>
                  <p className="text-xs text-gray-500 font-normal">Restrict entire IP addresses from accessing any part of the platform.</p>
                </div>
              </div>
              <button 
                onClick={() => setLocalSettings(s => s ? {...s, ip_blocking_enabled: !s.ip_blocking_enabled} : null)}
                className={`w-14 h-7 rounded-full transition-colors relative ${localSettings?.ip_blocking_enabled ? 'bg-gray-900' : 'bg-gray-200'}`}
              >
                <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${localSettings?.ip_blocking_enabled ? 'right-1' : 'left-1'}`} />
              </button>
            </div>
          </div>
        </section>
          ...
          <section className="space-y-6 animate-in slide-in-from-top-2">
            ...
            <div className="bg-gray-50/30 p-6 rounded-xl border border-gray-100 space-y-6">
              <div className="flex gap-3">
                ...
                <button 
                  onClick={addIp}
                  className="px-6 py-3 bg-gray-900 text-white rounded-[6px] hover:bg-black transition-all text-sm font-bold flex items-center gap-2"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {localSettings.blocked_ips.length === 0 ? (
                  <p className="text-sm text-gray-400 italic py-4 col-span-full text-center">No IP addresses blocked yet.</p>
                ) : (
                  localSettings.blocked_ips.map(ip => (
                    <div key={ip} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-[6px] shadow-sm">
