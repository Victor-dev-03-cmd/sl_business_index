...
export default function AdminAnalyticsPage() {
  ...
  const getStatColors = (color: string) => {
    switch (color) {
      ...
      case "brand-gold":
        return "bg-brand-sand/20 text-brand-gold";
      case "blue":
    ...
    <div className="min-h-full bg-gray-50/50 transition-colors">
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-2xl font-normal text-brand-dark">
              Global Insights
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              Real-time overview of platform performance and growth.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {statCards.map((stat) => (
            <div
              key={stat.name}
              className="bg-white p-6 rounded-[6px] border border-gray-300 shadow-sm hover:shadow-md transition-shadow group"
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`p-2.5 rounded-[6px] ${getStatColors(stat.color)} group-hover:scale-110 transition-transform`}
                >
                  <stat.icon size={20} />
                </div>
                <div
                  className={`flex items-center gap-1 text-[11px] font-normal ${stat.trending === "up" ? "text-brand-blue" : "text-red-600"}`}
                >
                  {stat.trending === "up" ? (
        ...
        <div className="mt-8 bg-white rounded-[6px] border border-gray-300 p-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            ...
            <div className="space-y-2 border-y border-gray-100 md:border-x md:border-y-0 py-6 md:py-0">
              <p className="text-xs text-gray-400 uppercase tracking-widest">
                Active Users
              </p>
