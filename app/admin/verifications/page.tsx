...
export default function VerificationsPage() {
  ...
  const fetchVerifications = async () => {
    ...
    try {
      let query = supabase
        .from('verifications')
        .select('*, businesses(name, logo_url, owner_name, email, phone, owner_id)')
  ...
  const handleAction = async (id: string, business_id: string, owner_id: string, status: 'approved' | 'rejected') => {
    setSubmitting(id);
    try {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Review and manage vendor identity verification requests.</p>
      </div>
      ...
      <div className="flex gap-6 border-b border-gray-200">
        ...
        >
          <Clock size={16} />
          Pending Requests
          {activeTab === 'pending' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-blue animate-in fade-in" />}
        </button>
        <button
        ...
        >
          <History size={16} />
          History
          {activeTab === 'history' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-brand-blue animate-in fade-in" />}
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="px-4 py-2 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-100 uppercase tracking-wider">
          {verifications.length} {activeTab === 'pending' ? 'Pending' : 'Total'} Requests
        </div>
      </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-300">
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Business Info</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Document</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">
                    {activeTab === 'pending' ? 'Requested Date' : 'Status & Date'}
                  </th>
                  {activeTab === 'pending' && (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-brand-dark flex items-center justify-center text-white font-bold shrink-0 overflow-hidden">
                          {v.businesses.logo_url ? <img src={v.businesses.logo_url} alt={v.businesses.name} className="h-full w-full object-cover" /> : <Building2 size={18} />}
                        </div>
                        <div>
                          <div className="text-sm font-bold text-gray-900">{v.businesses.name || 'N/A'}</div>
                      ...
                      {v.br_document_url ? (
                        <div className="relative group h-16 w-24 rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex items-center justify-center">
                          <img 
                            ...
                            onClick={() => window.open(v.br_document_url, '_blank')}
                          />
                      ...
                      {activeTab === 'history' ? (
                        <div>
                          <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                            v.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                          }`}>
                            {v.status}
                          </span>
                    ...
                    {activeTab === 'pending' && (
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => handleAction(v.id, v.business_id, v.businesses.owner_id, 'approved')}
                          ...
                          <button
                            onClick={() => handleAction(v.id, v.business_id, v.businesses.owner_id, 'rejected')}
                            disabled={submitting === v.id}
