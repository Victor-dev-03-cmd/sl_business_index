...
export default function AuthButton({ user }: { user?: any | null }) {
  ...
  if (user === undefined) { // Still loading from Navbar
    ...
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-64 bg-white p-2 shadow-2xl border border-gray-100 rounded-[6px]" align="end">
          ...
          <DropdownMenuItem asChild className="cursor-pointer py-2.5">
            <Link href={isAdminOrCeo ? "/admin/settings" : "/vendor/settings"} className="flex items-center w-full">
  ...
  return (
    <Link href="/login" className="px-6 py-2.5 text-sm text-white bg-brand-dark rounded-[6px] hover:bg-brand-blue transition-colors">
      Sign In
    </Link>
  );
}