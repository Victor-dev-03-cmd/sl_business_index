...
-- Update get_nearby_businesses RPC to search in city, address, and detailed_address
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    b.id, b.slug, b.name, b.category, b.description, b.address, b.detailed_address, b.city, b.phone, b.email,
  ...
  WHERE
    ...
    AND (
      ...
      OR b.city ILIKE '%' || clean_search || '%'
      OR b.address ILIKE '%' || clean_search || '%'
      OR b.detailed_address ILIKE '%' || clean_search || '%'
    )
  ORDER BY
    search_rank DESC,
